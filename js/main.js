...
select.addEventListener('change', event => {//Inside the select box we're adding an event listener so each time something changes pass in an event
    // console.log(event.target.value) //It helps just to see if you're really getting the right value
    let url = `https://dog.ceo/api/breed/${event.target.value}/images/random`
    // console.log(url); //It helps just to see if you're really getting it right
    getDoggoImg(url)
    doggoInfo.assignMF() //Because it's a method inside doggoInfo object
    doggoInfo.assignAge()
...
const img = document.querySelector('.dog-img') //replace the content of the img tag

const getDoggoImg = url => { //passing in URL as a parameter and do similar as fetch (getting json and then a msg from the json)
    fetch(url)//Going to the API url above
        .then(res => {
            return res.json() //get json message back
        }) //assuming that fetching the json was successful
        .then(data => {
            img.src = data.message //extract message from json & assign that data to something (in this case the img tag) as new source
...
const doggoInfo = {
    ...
    assignMF() {
        ...
        if (x) {
            this.MF = 'Female';
            this.assignName(this.fNames) //Calls a new method
        } else {
            this.MF = 'Male'
